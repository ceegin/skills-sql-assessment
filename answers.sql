...
SELECT models.name, brands.name, headquarters FROM models JOIN brands USING (brand_id) WHERE models.name = 'Mustang' AND brands.name = 'Ford';
...
SELECT COUNT(*) FROM models JOIN brands USING (brand_id) WHERE brands.name = 'Ford';
...
SELECT models.year, models.name, model_id, awards.name FROM models
LEFT JOIN awards ON (models.model_id = awards.winner_id) WHERE models.year >=1964;
